...
pub fn evaluate(levels: &[u32]) -> bool {
    let (safe, dir): (Vec<bool>, Vec<bool>) = levels.into_iter().tuple_windows().map(|(x, y)| {
        check_safe(x, y)
    }).collect();
...
pub fn check_safe(x: &u32, y: &u32) -> (bool, bool) {
    ...
    let diff_ok = difference >= 1  && difference <= 3;
    (diff_ok, is_increasing)
}

pub fn part_one(input: &str) -> Option<u32> {
    Some(input.lines().filter_map(|line| {
        let levels = line.split_whitespace().map(|c| c.parse::<u32>().unwrap()).collect_vec();
        evaluate(&levels).then_some(())
    }).count() as u32)
}

pub fn part_two(input: &str) -> Option<u32> {
    let r = input.lines().filter_map(|(line)| {
        let levels = line.split_whitespace().map(|c| c.parse::<u32>().unwrap()).collect_vec();
        let all_safe = evaluate(&levels);
        if all_safe  {
            return Some(())
        }
        for p in 0..levels.len() { //todo: check only problem indexes
